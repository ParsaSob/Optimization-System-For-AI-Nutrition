...
class SingleMealOptimizer:
    """
    Specialized optimizer for single meal optimization with RAG enhancement
    Uses mathematical optimization algorithms to meet target macros
    """
    ...
    def _evaluate_genetic_fitness(self, individual):
        ...
        # This is a placeholder that will be overridden in the optimization methods
        return (0.0,)
    ...
    def _calculate_current_totals(self, ingredients: List[Dict]) -> Dict:
        ...
        for ingredient in ingredients:
            amount = ingredient['original_amount']
            totals['calories'] += (ingredient['calories_per_100g'] * amount) / 100
            totals['protein'] += (ingredient['protein_per_100g'] * amount) / 100
            totals['carbs'] += (ingredient['carbs_per_100g'] * amount) / 100
            totals['fat'] += (ingredient['fat_per_100g'] * amount) / 100
    ...
    def _filter_ingredients_by_preferences(
        ...
        if 'vegetarian' in preferences.get('dietary_restrictions', []):
            filtered = [ing for ing in filtered if 'meat' not in ing.category.lower()]
        
        if 'vegan' in preferences.get('dietary_restrictions', []):
            filtered = [ing for ing in filtered if 'dairy' not in ing.category.lower()]
        
        # Filter by allergies
        allergies = preferences.get('allergies', [])
        for allergy in allergies:
            filtered = [ing for ing in filtered if allergy.lower() not in ing.name.lower()]
    ...
    def _optimize_linear_programming(
        ...
        try:
            ...
            quantities = [LpVariable(f"q_{i}", 0, None) for i in range(len(ingredients))]
            
            # Objective: minimize total deviation from targets
            prob += lpSum([
                abs(quantities[i] * ingredients[i]['calories_per_100g'] / 100 - target_macros['calories']) +
                abs(quantities[i] * ingredients[i]['protein_per_100g'] / 100 - target_macros['protein']) +
                abs(quantities[i] * ingredients[i]['carbs_per_100g'] / 100 - target_macros['carbs']) +
                abs(quantities[i] * ingredients[i]['fat_per_100g'] / 100 - target_macros['fat'])
                for i in range(len(ingredients))
            ])
            
            # Constraints: meet targets within ±10% tolerance
            tolerance = 0.1
            prob += lpSum([quantities[i] * ingredients[i]['calories_per_100g'] / 100 
                          for i in range(len(ingredients))]) >= target_macros['calories'] * (1 - tolerance)
            prob += lpSum([quantities[i] * ingredients[i]['calories_per_100g'] / 100 
                          for i in range(len(ingredients))]) <= target_macros['calories'] * (1 + tolerance)
            
            prob += lpSum([quantities[i] * ingredients[i]['protein_per_100g'] / 100 
                          for i in range(len(ingredients))]) >= target_macros['protein'] * (1 - tolerance)
            prob += lpSum([quantities[i] * ingredients[i]['protein_per_100g'] / 100 
                          for i in range(len(ingredients))]) <= target_macros['protein'] * (1 + tolerance)
            
            prob += lpSum([quantities[i] * ingredients[i]['carbs_per_100g'] / 100 
                          for i in range(len(ingredients))]) >= target_macros['carbs'] * (1 - tolerance)
            prob += lpSum([quantities[i] * ingredients[i]['carbs_per_100g'] / 100 
                          for i in range(len(ingredients))]) <= target_macros['carbs'] * (1 + tolerance)
            
            prob += lpSum([quantities[i] * ingredients[i]['fat_per_100g'] / 100 
                          for i in range(len(ingredients))]) >= target_macros['fat'] * (1 - tolerance)
            prob += lpSum([quantities[i] * ingredients[i]['fat_per_100g'] / 100 
                          for i in range(len(ingredients))]) <= target_macros['fat'] * (1 + tolerance)
            ...
            if prob.status == 1:  # Optimal solution found
                quantities_values = [value(quantities[i]) for i in range(len(ingredients))]
                return {
                    'success': True,
                ...
                return {'success': False, 'method': 'Linear Programming (PuLP)', 'quantities': []}
    ...
    def _optimize_hybrid(
        ...
        try:
            ...
            # Final fallback to genetic algorithm
    ...
    def _calculate_enhancement_details(
        ...
        original_count = len([ing for ing in rag_ingredients if ing['source'] == 'rag'])
